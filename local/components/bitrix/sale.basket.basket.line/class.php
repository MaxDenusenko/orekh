...
class SaleBasketLineComponent extends CBitrixComponent
{
	...
	protected function setCountItemForCart()
    {
        if (preg_match('/^[0-9]+$/', $_POST["sbblSetCountForCart"]) !== 1)
            return;

        if (!($userFilter = $this->getUserFilter()))
            return;

        $count = round(abs($_POST["count"])*1);
        $count = $count > 0 ? $count : 1;

        $basket = Sale\Basket::loadItemsForFUser(Sale\Fuser::getId(), Bitrix\Main\Context::getCurrent()->getSite());
        $arItem = $basket->getItemById($_POST['sbblSetCountForCart']);

        if ($arItem) {

            $arItem->setField('QUANTITY', $count);
            $basket->save();
        }
    }

	protected function pickUpItemFromCart()
    {
        if (preg_match('/^[0-9]+$/', $_POST["sbblPickUpItemFromCart"]) !== 1)
            return;
        ...
        $basket = Sale\Basket::loadItemsForFUser(Sale\Fuser::getId(), Bitrix\Main\Context::getCurrent()->getSite());
        $arItem = $basket->getItemById($_POST['sbblPickUpItemFromCart']);

        if ($arItem && $arItem->getQuantity() > 1) {

            $arItem->setField('QUANTITY', $arItem->getQuantity() - 1);
            $basket->save();
        }
    }

	protected function appendItemToCart()
    {
        if (preg_match('/^[0-9]+$/', $_POST["sbblAppendItemToCart"]) !== 1)
            return;
        ...
        $basket = Sale\Basket::loadItemsForFUser(Sale\Fuser::getId(), Bitrix\Main\Context::getCurrent()->getSite());
        $arItem = $basket->getItemById($_POST['sbblAppendItemToCart']);
	...
	public function executeComponent()
	{
        ...
        if (isset($_POST['sbblAppendItemToCart']))
            $this->appendItemToCart();

        if (isset($_POST['sbblPickUpItemFromCart']))
            $this->pickUpItemFromCart();

        if (isset($_POST['sbblSetCountForCart']))
            $this->setCountItemForCart();
